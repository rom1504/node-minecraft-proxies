...
srv.on('login', function(client) {
  ...
  targetClient.on("login",function(){
    offlineToOnline[targetClient.uuid]=client.uuid;
  });
      ...
      if(!endedClient) {
        if(meta.name=="player_info")
          data.data.forEach(function(player){
            player.UUID=offlineToOnline[player.UUID];
            if(data.action==0)
              player.properties=client.profile.properties
                  .map(property => ({
                  name:property.name,
